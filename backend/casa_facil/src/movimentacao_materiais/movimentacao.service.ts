  ...
  async create(dto: CreateMovimentacaoDto, userId: number) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (tipo_movimentacao === 'O') {
        ...
        if (!material || material.qt_estoque < qtde) {
      ...
      await tx.materiais_estoque.update({
        where: { id: materialId },
        data: { qt_estoque: stockUpdateOperation },
      });
  ...
  async remove(id: number) {
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.materiais_estoque.update({
        ...
        data: { qt_estoque: stockRevertOperation },
      });
  ...
  async update(id: number, dto: UpdateMovimentacaoDto, userId: number) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (newTipo === 'O') {
        ...
        if (!material || material.qt_estoque < newQtde) {
  ...
  findAll() {
    return this.prisma.movimentacao_materiais.findMany({
      ...
      include: {
        ...
        materiais_estoque: { select: { nome: true } },
      },
    });
  }
